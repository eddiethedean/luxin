...
def test_sorted_unique_mixed_types_no_cross_compare_error():
    from luxin.components.filters import _sorted_unique_for_multiselect

    s = pd.Series([1, "a"])
    vals = _sorted_unique_for_multiselect(s)
    assert len(vals) == 2
    assert sorted([type(v).__name__ for v in vals]) == ["int", "str"]
