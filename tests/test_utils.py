...
def test_normalize_group_key_numpy_integer():
    """Numpy group keys align with Python ints for dict lookup."""
    assert normalize_group_key((np.int64(1),)) == (1,)
    assert normalize_group_key((np.int32(2), 3)) == (2, 3)


def test_normalize_group_key_datetime():
    ts = pd.Timestamp("2020-01-01")
    assert normalize_group_key((ts,)) == (ts,)
    dt = datetime(2020, 1, 1)
    assert normalize_group_key((dt,)) == (ts,)


def test_finalize_source_mapping_normalizes_numpy_scalar_key():
    m = {(np.int64(1),): [2, 1, 2]}
    out = finalize_source_mapping(m)
    assert out == {(1,): [1, 2]}
