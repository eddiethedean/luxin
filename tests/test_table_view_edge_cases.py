...
@patch("luxin.components.table_view.st")
def test_render_table_view_streamlit_too_old(mock_st, _mock_dataframe_supported):
    agg_df = pd.DataFrame({"category": ["A"], "value": [1]})
    detail_df = agg_df.copy()
    mock_st.error = MagicMock()
    mock_st.header = MagicMock()
    render_table_view(agg_df, detail_df, {("A",): [0]}, ["category"])
...
@patch("luxin.components.table_view.render_filters")
@patch("luxin.components.table_view.render_export_buttons")
def test_render_table_view_unnamed_index_aligns_groupby_column(
    mock_export,
    mock_filters,
    mock_st,
    mock_details,
):
    """Unnamed index holding group keys gets columns renamed to ``groupby_cols``."""
    agg_df = pd.DataFrame({"value": [30, 70]}, index=pd.Index(["A", "B"], name=None))
    detail_df = pd.DataFrame(
        {"category": ["A", "A", "B", "B"], "value": [10, 20, 30, 40]}
    )
    source_mapping = {("A",): [0, 1], ("B",): [2, 3]}
