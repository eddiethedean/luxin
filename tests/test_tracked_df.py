...
def test_groupby_numpy_array_of_column_names():
    import numpy as np

    df = TrackedDataFrame({"category": ["A", "B"], "value": [1, 2]})
    result = df.groupby(np.array(["category"])).agg({"value": "sum"})
    assert result._groupby_cols == ["category"]


def test_groupby_integer_labels_use_canonical_keys():
    import numpy as np

    df = TrackedDataFrame({"k": np.array([1, 1, 2], dtype=np.int64), "v": [10, 20, 30]})
    result = df.groupby("k").agg({"v": "sum"})
    assert (1,) in result._source_mapping and (2,) in result._source_mapping


def test_groupby_pd_grouper_not_supported():
